namespace PE.Battle.Abilities {
  ...
  export function OnSwitchInEffects(pokemon: Battle.Battler, onactive: boolean) {
    ...
    if (onactive) {
      ...
      if (!($Battle.weather in PrimalWeathers)) {
        if (
          pokemon.hasAbility(Abilitydex.DRIZZLE) &&
          ($Battle.weather !== Weathers.RainDance || $Battle.weatherDuration < 0)
        ) {
        ...
        if (
          pokemon.hasAbility(Abilitydex.DROUGHT) &&
          ($Battle.weather !== Weathers.SunnyDay || $Battle.weatherDuration < 0)
        ) {
        ...
        if (
          pokemon.hasAbility(Abilitydex.SANDSTREAM) &&
          ($Battle.weather !== Weathers.SandStorm || $Battle.weatherDuration < 0)
        ) {
        ...
        if (
          pokemon.hasAbility(Abilitydex.SNOWWARNING) &&
          ($Battle.weather != Weathers.Hail || $Battle.weatherDuration != -1)
        ) {
    ...
    if (pokemon.hasAbility(Abilitydex.TRACE)) {
      ...
      for (const battler of $Battle.actives) {
        if (pokemon.isOpposing(battler.index)) {
          if (
            !battler.hasAbilityIn([
              Abilitydex.TRACE,
              Abilitydex.MULTITYPE,
              Abilitydex.FLOWERGIFT,
              Abilitydex.IMPOSTER,
              Abilitydex.STANCECHANGE,
              Abilitydex.RKSSYSTEM,
              Abilitydex.ILLUSION,
              Abilitydex.ZENMODE,
              Abilitydex.STANCECHANGE,
              Abilitydex.POWEROFALCHEMY,
              Abilitydex.RECEIVER,
              Abilitydex.SCHOOLING,
              Abilitydex.COMATOSE,
              Abilitydex.SHIELDSDOWN,
              Abilitydex.DISGUISE,
              Abilitydex.BATTLEBOND,
              Abilitydex.POWERCONSTRUCT
            ])
          ) {
  ...
  export function OnDealingDamageEffects(move: Moves.Move, user: Battler, target: Battler, damage: number) {
    ...
    if (move.isContactMove() && !target.damageState.substitute && !user.hasAbility(Abilitydex.LONGREACH, true)) {
      ...
      if (target.hasAbility(Abilitydex.AFTERMATH, true) && target.isFainted() && !user.isFainted()) {
        if (
          !$Battle.checkGlobalAbility(Abilitydex.DAMP) &&
          !user.hasMoldBreaker() &&
          !user.hasAbility(Abilitydex.MAGICGUARD)
        ) {
      ...
      if (target.hasAbility(Abilitydex.CUTECHARM) && Utils.chance(30)) {
        if (!user.isFainted() && user.canAttract(target, false)) {
          ...
          $Battle.showMessage(
            i18n._("%1's %2 made %3 fall in love", target.name, Abilities.name(target.ability), user.name)
          );
  ...
  export function moveHitTypeImmunity(move: Battle.Moves.Move, attacker: Battle.Battler, opponent: Battle.Battler) {
    ...
    if (opponent.hasAbility(Abilitydex.SAPSIPPER) && movetype === Types.GRASS) {
      ...
      if (opponent.canIncreaseStatStage(Stats.Attack, opponent)) {
        ...
        $Battle.showMessage(
          i18n._("%1's %2 made %3 ineffective!", opponent.name, Battle.Abilities.name(opponent.ability), self.name)
        );
      }
      return true;
    }
    if (
      (opponent.hasAbility(Abilitydex.STORMDRAIN) && movetype === Types.WATER) ||
      (opponent.hasAbility(Abilitydex.LIGHTNINGROD) && movetype === Types.ELECTRIC)
    ) {
      console.log(
        "[Ability triggered] ${opponent.name}'s ${AbilitiesEffetcs.name(opponent.ability)} (made ${move.name} ineffective)"
      );
    ...
    if (
      (opponent.hasAbilityIn([Abilitydex.DRYSKIN, Abilitydex.WATERABSORB]) && movetype === Types.WATER) ||
      ...
      if (!healed) {
        $Battle.showMessage(
          i18n._("%1's %2 made %3 useless!", opponent.name, Battle.Abilities.name(opponent.ability), move.name)
        );
      }
      return true;
    }
    if (opponent.hasAbility(Abilitydex.FLASHFIRE) && movetype === Types.FIRE) {
      ...
      if (!opponent.effects.FlashFire) {
        ...
        $Battle.showMessage(
          i18n._("%1's %2 raised its Fire power!", opponent.name, Battle.Abilities.name(opponent.ability))
        );
      } else {
        $Battle.showMessage(
          i18n._("%1's %2 made %3 ineffective!", opponent.name, Battle.Abilities.name(opponent.ability), self.name)
  ...
  export function AfterHitEffects(user: Battler, target: Battler, thismove: Moves.Move, turneffects) {
    ...
    if (!user.hasAbility(Abilitydex.SHEERFORCE)) {
      ...
      if (target.hasItem('REDCARD') && $Battle.canSwitch(user.index, -1, false)) {
        ...
        $Battle.showMessage(
          i18n._('%1 held up its %2 against the %3!', target.name, Items.name(target.item), user.name)
        );
        target.consumeItem();
        // Eject Button
      ...
      if (target.hasAbility(Abilitydex.COLORCHANGE) && !target.hasType(movetype)) {
        ...
        $Battle.showMessage(
          i18n._("%1's %2 made it the %3 type!", target.name, Abilities.name(target.ability), movetype)
        );
      }
    }
    // Moxie
    ...
    if (user.hasAbility(Abilitydex.MAGICIAN)) {
      if (
        ...
        !target.hasAbility(Abilitydex.STICKYHOLD) &&
        !$Battle.isUnlosableItem(target, target.item) &&
        !$Battle.isUnlosableItem(user, target.item) &&
        ($Battle.opponent || !$Battle.isOpposing(user.index))
      ) {
    ...
    if (target.hasAbility(Abilitydex.PICKPOCKET)) {
      if (
        ...
        user.effects.Substitute == 0 &&
        target.effects.Substitute == 0 &&
        !user.hasAbility(Abilitydex.STICKYHOLD) &&
        !$Battle.isUnlosableItem(user, user.item) &&
        !$Battle.isUnlosableItem(target, user.item) &&
        ($Battle.opponent || !$Battle.isOpposing(target.index))
      ) {
        target.item = user.item;
        user.item = undefined;
        user.effects.Unburden = true;
        if (
          !$Battle.opponent && // In a wild battle
      ...
      $Battle.showMessage(i18n._("%1 pickpocketed %2's %3!", target.name, user.name, Items.name(target.item)));
      console.log(
        "[Ability triggered] ${target.name}'s Pickpocket stole ${user.name(true)}'s ${Items.name(target.item)}"
      );
    }
  }
  ...
  export function AbilityCureCheck(pokemon: Battler) {
    ...
    if (pokemon.effects.Confusion > 0 && pokemon.hasAbility(Abilitydex.OWNTEMPO)) {
      ...
      $Battle.showMessage(
        i18n._("%1's %2 snapped it out of its confusion!", pokemon.name, Abilities.name(pokemon.ability))
      );
    }
    if (pokemon.effects.Attract >= 0 && pokemon.hasAbility(Abilitydex.OBLIVIOUS)) {
      ...
      pokemon.cureAttract();
      $Battle.showMessage(
        i18n._("%1's %2 cured its infatuation status!", pokemon.name, Abilities.name(pokemon.ability))
      );
    }
  ...
  export function SpeedStatEffects(pokemon: Battler, speed: number) {
    if (
      pokemon.hasAbility(Abilitydex.CHLOROPHYLL) &&
      ($Battle.weather === Weathers.SunnyDay || $Battle.weather === Weathers.HarshSun)
    )
      speed *= 2;
    else if (pokemon.hasAbility(Abilitydex.QUICKFEET) && pokemon.status !== Statuses.Healthy)
      ...
      pokemon.hasAbility(Abilitydex.SWIFTSWIM) &&
      ($Battle.weather === Weathers.RainDance || $Battle.weather === Weathers.HeavyRain)
    )
      speed *= 2;
...
namespace PE {
  ...
  RegisterAbilityEffects(Abilitydex.DRIZZLE, {
    onSwitchIn: function(pokemon) {
      ...
      $Battle.setWeather(Weathers.RainDance, weatherduration);
      $Battle.showAbilityIndicator(pokemon);
      $Battle.showMessage(i18n._("%1's %2 made it rain!", pokemon.name, Battle.Abilities.name(pokemon.ability)));
      console.log(`[Ability triggered] ${pokemon.name} Drizzle made it rain`);
    }
  });
  ...
  RegisterAbilityEffects(Abilitydex.FULLMETALBODY, {
    onBoost: function(stat, target, source) {
