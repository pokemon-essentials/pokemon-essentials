namespace PE {
  export const CHARACTERS_PER_LINE = 55;
  ...
  export class Battle_Manager {
    ...
    static useMove(move: PE.Battle.Moves.Move, target) {
      ...
      console.log(`: [${move.name}] accuracy ${move.accuracy}`);
    ...
    static calculateDamage(source: Battle_Battler, target: Battle_Battler, move: PE.Battle.Moves.Move) {
      ...
      console.log(`: [${move.name}] Base Power ${basePower}`);
    ...
    static setWeather(weather: WEATHERS, duration) {
      EventManager.emit("SET_WEATHER", weather);
      let success = EventManager.run("SetWeather", undefined, undefined, { weather: weather });
...
const $BattleManager = PE.Battle_Manager;
