...
const Phase = {None: 0, Init: 1, ActionSelection: 2, MoveSelection: 3, Animation: 4};
const WaitMode = {None: 0, Animation: 1, AbilitySing: 2};
const ActionChoices = {UseMove: 0, UseItem: 1, Switch: 2};
...
class Manager {
  ...
  /**
   * @param opponets {Trainers.Trainer[]}
   * @param allies {Trainers.Trainer[]}
   */
  static setup(opponets, allies) {
    this._queue = [];
  ...
  /**
   * @param pokemon {Battler}
   * @param item {string}
   */
  static isUnlosableItem(pokemon, item) {
   ...
   */
  static checkGlobalAbility(ability) {
  ...
  static canShowMovesSelection(index) {
    let pokemon = this.battlers[index];
  ...
  static canChooseMove(index, move, showMessages, sleeptalk) {
    sleeptalk = sleeptalk || false;
    let pokemon = this.battlers[index];
    if (!move) return;
  ...
  static getDamage(source, target, move) {
  ...
  static showAbilityIndicator(pokemon) {
  ...
  static nextPickupUse() {
    throw new Error('not implemented');
  }

  static recoverHPAnimation(index) {
    this.push(() => {
