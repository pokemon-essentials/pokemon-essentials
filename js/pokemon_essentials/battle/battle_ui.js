...
PE.Battle.UI.AbilityIndicator = class AbilityIndicator extends Sprite {
  constructor(ability) {
    ...
    this.bitmap.drawText(PE.Battle.Abilities.name(ability), 0, 0, 224, 32, 'center');
    this.ticks = 0;
  }
...
PE.Battle.UI.HPBar = class HPBar extends Sprite {
  ...
  create() {
    ...
    if (!this.foe) {
      ...
      this.indicator.bitmap.drawText(
        this.pokemon.hp + '/' + this.pokemon.hp,
        this._x + 32,
        this.box.y + 8,
        200,
        24,
...
PE.Battle.UI._MovesSelection = class _MovesSelection extends Sprite {
  ...
  createBackground() {
    ...
    var backtext = new Sprite(new Bitmap(96, 48));
    backtext.bitmap.fontSize = 24;
    backtext.bitmap.outlineWidth = 4;
    backtext.bitmap.outlineColor = '#818181';
    backtext.bitmap.drawText(i18n._('BACK'), 4, 0, 96, 48, 'center');
    backtext.x = 0;
    backtext.y = Graphics.height - 4;
    backtext.anchor.y = 1;
    this.addChild(backtext);

    this._backButton.setBlendColor([0, 0, 0, 115]);
    backtext.setBlendColor([0, 0, 0, 115]);
  }
  createButtons() {
    var i = 0;
  ...
  updateInput() {
    ...
    if (Input.isTriggered('down')) {
      ...
      if (PE.Battle.UI.movesInx >= this._moves.length)
        PE.Battle.UI.movesInx = Math.abs(this._moves.length - PE.Battle.UI.movesInx);
      this._moves[PE.Battle.UI.movesInx].activate();
      SoundManager.playCursor();
      return;
    }
